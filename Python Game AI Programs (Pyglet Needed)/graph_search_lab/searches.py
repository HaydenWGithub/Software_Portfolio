...
def SearchRandom(graph, source_idx, target_idx, limit=0):
    ''' Random Search. Picks a node to explore randomly. '''
    closed = set()  # set - of visited nodes
    route = {}  # dict of {to:from} items to find our way home
    open = []  # use a list as a queue of the current leaf edges
    steps = 0  # if limit

    # add the starting source as an edge to self
    open.append(source_idx)
    route[source_idx] = source_idx  # to:from
    # search loop
    while len(open):
        steps += 1
        leaf = random.choice(open)  # get a random node to investigate
        open.remove(leaf)
        closed.add(leaf)
        if leaf == target_idx:
            break
