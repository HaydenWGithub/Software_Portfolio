...
#this value was initially set to 10.0, but according to the edge cost matrix, this seems to be the largest cost value, and not the minimum, which is 1.0
...
class BoxWorld(object):
    ...
    def __init__(self, nx, ny, cx, cy):
        ...
        self.start = None
        self.target = None
        #establish an array to store agents
        self.agents = []
    ...
    #incorporating the upate function from path planning minus the paused condition
    def update(self, delta):
        for agent in self.agents:
            agent.update(delta)
        #incorporating the agent rendering
    ...
    def get_path_as_points(self):
        '''Return the path as a list of Vector2D objects, where each point is at
        the center of the corresponding box on the path. If no path has been found,
        return None.'''
        if self.path and self.path.path:
            return [Vector2D(self.boxes[i]._vc.x, self.boxes[i]._vc.y) for i in self.path.path]
        else:
            return None
    
    def print_path_points(self):
        path_points = self.get_path_as_points()
        if path_points is not None:
            for point in path_points:
                print(f'Point X: {point.x}, Point Y: {point.y}')
        else:
            print("No path points available.")
