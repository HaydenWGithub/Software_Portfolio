...
class World(object):
    ...
    def render(self):
        ...
        for circle in self.circles:
            ...
            if (circle.position - circle.target).length() > 1.0:
                valid_circles.append(circle)
                egi.blue_pen()
                egi.circle(circle.position, circle.radius)
            
            self.circles = valid_circles
...
class Circle(object):
    ...
    def isColliding(self, x, y):
        return (x - self.position.x)**2 + (y - self.position.y)**2 < self.radius**2
